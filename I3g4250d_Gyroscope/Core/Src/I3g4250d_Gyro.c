...
#define GYRO_REG_CTRL2_SETTINGS     0x03
#define GYRO_REG_CTRL3_SETTINGS     0x80
#define GYRO_REG_CTRL4_SETTINGS     0x20
#define GYRO_REG_CTRL5_SETTINGS     0x18
#define GYRO_REG_INT1_SETTINGS      0x7F
#define GYRO_YL						0x7F
...
uint8_t zege;
uint64_t timestamp;
uint64_t Int_microseconds;
uint64_t Poll_microseconds;
...
void I3g4250d_Init(SPI_HandleTypeDef spi_handle)
{
	...
	// Write required setting to INT1
	Gyro_SPI_Write(GYRO_REG_INT1, GYRO_REG_INT1_SETTINGS);

	//Gyro_SPI_Write(GYRO_INT1_THS_YL, GYRO_YL);
...
void I3g4250d_Loop(void)
{
	...
	static uint64_t ull_DataNowTimestamp;
	...
	zege=Gyro_SPI_Read(GYRO_INT_STATUS);
	...
	{
		Poll_microseconds = SysTick_Elapsed_MicroSeconds(ull_DataNowTimestamp);
...
{
	Int_microseconds = SysTick_Elapsed_MicroSeconds(timestamp);
